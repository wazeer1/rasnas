...
const OrderDetails: React.FC<{ className?: string }> = ({
  ...
  const order = data?.app_data?.data;
  ...
  return (
    <div className={className}>
      ...
      <table className="w-full text-sm font-semibold text-heading lg:text-base">
        <thead>
          <tr>
            <th className="w-1/2 p-4  ltr:text-left rtl:text-right ltr:first:rounded-tl-md rtl:first:rounded-tr-md">
              {t("text-product")}
            </th>
            <th className="w-1/2 p-4  ltr:text-left rtl:text-right ltr:last:rounded-tr-md rtl:last:rounded-tl-md">
              {t("text-total")}
            </th>
          </tr>
        </thead>
